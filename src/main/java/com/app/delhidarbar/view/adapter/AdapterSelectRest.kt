...
class AdapterSelectRest(private val context: Context, private val restaurants: List<Restaurant>) : RecyclerView.Adapter<AdapterSelectRest.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.restaurent_name.text = restaurants[position].name
        holder.restaurent_address.text = restaurants[position].address
        holder.progress_bar.visibility = View.VISIBLE
      //Picasso.get().load(restaurants[position].image).into(holder.restaurentImage)

       // DelhiDarbar.instance!!.commonMethods.setImageWithLoader(restaurants[position].image, holder.restaurentImage, holder.progress_bar)
